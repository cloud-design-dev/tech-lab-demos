...
storage_data = {
    "current_step": 6,  # Updated to step 6 for health probe demo
    "steps": [
        ...
        {"id": 5, "name": "Resource Limits & Scaling", "status": "completed"},
        {"id": 6, "name": "Configure Health Probes", "status": "current"}
    ],
    "deployment_info": {
        "app_name": "OpenShift Demo App - V4 Health Probes",
        "version": "4.0.0",
...
def check_deployment_probes():
    """Check if health probes are configured in the deployment"""
    try:
        # Try to get deployment configuration via oc command
        result = subprocess.run(['oc', 'get', 'deployment', 'demo-app-v4', '-o', 'json'], 
        ...
        if result.returncode == 0:
            ...
            if containers:
                container = containers[0]  # First container
                has_liveness = 'livenessProbe' in container
                has_readiness = 'readinessProbe' in container  
                has_startup = 'startupProbe' in container
                
                return {
                    "probes_configured": has_liveness or has_readiness or has_startup,
                    "liveness_probe": has_liveness,
                    "readiness_probe": has_readiness,
                    "startup_probe": has_startup,
                    "method": "oc_command"
                }
        
        return {
            "probes_configured": False,
            "liveness_probe": False,
            "readiness_probe": False,
            "startup_probe": False,
            "method": "detection_failed",
            "error": "Could not check deployment configuration"
        }
...
def get_probe_status():
    """Get current health probe configuration status"""
    probe_info = check_deployment_probes()
    
    return jsonify({
        "probes_configured": probe_info["probes_configured"],
        "probe_details": {
            "liveness": probe_info["liveness_probe"],
            "readiness": probe_info["readiness_probe"],
            "startup": probe_info["startup_probe"]
        },
        "endpoints_available": {
            "liveness": "/api/health",
            "readiness": "/api/ready", 
        ...
        "step_6_complete": probe_info["probes_configured"],
        "detection_method": probe_info["method"],
        "timestamp": datetime.now().isoformat()
    })
